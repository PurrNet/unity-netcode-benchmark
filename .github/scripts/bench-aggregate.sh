...
if [ -f "$SERVER_FILE" ]; then
  jq -r "$JQ_LIB"'
    ...
    | "Window: '"$WINDOW"'s · Objects: '"$OBJECTS"' · Measured clients: '"${#CLIENT_FILES[@]}"' · Server CPU: \($run.cpuModel) x\($run.cpuCount) · Tick: \($run.tickRate) Hz · \(if $run.devBuild then "Development" else "Release" end) build · Connected at start: \($run.connectedAtStart)/\($run.expectedClients)"
      ...
      + "\n\n### Server\n\n"
      + "| Test | Objects | Conns | Down on-wire | Per-conn down | Up on-wire | Pkts out/s | Inputs in/s | CPU | CPU − idle | Avg frame | p95 | p99 | GC | Heap | Peak RSS |\n"
      + "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n"
      + ( [ $run.tests[]
            | (if .connections > 0 then .txBytesPerSec / .connections else null end) as $perConn
            | "| \(.name)\(if .truncated then " ⚠️" else "" end) | \(.objects) | \(.connections) | \(.txBytesPerSec|hbR) | \($perConn|hbR) | \(.rxBytesPerSec|hbR) | \(.txPacketsPerSec|floor) | \(if (.inputsPerSec // 0) > 0 then (.inputsPerSec|floor) else "-" end) | \(.cpuPercent|r1)% | \((.cpuPercent - $idle)|r1)% | \(.avgFrameMs|r2) ms | \(.p95FrameMs|r2) ms | \(.p99FrameMs|r2) ms | \(.gcCollections) | \(.managedHeapBytes|mb) | \(.peakRssBytes|mb) |"
          ] | join("\n") )
